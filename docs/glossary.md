...
**Entity Lifecycle**
The set of callback hooks (`beforeInsert`, `afterInsert`, `beforeUpdate`, `afterUpdate`, `beforeUpsert`, `afterUpsert`, `beforeRemove`, `afterRemove`) that fire around mutation operations. An "after" callback observes what the calling method reports to its caller. Implemented via the `EntityCallback<E>` interface, these hooks enable cross-cutting concerns like auditing and validation. An entity declares the callbacks that belong to its own definition with `@EntityCallbacks`, and Storm creates them; a callback with dependencies is created by the application and registered on the template. See [Entity Lifecycle](entity-lifecycle.md).
