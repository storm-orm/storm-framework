...
2. Write tests covering the key user flows:
   ...
   - **Search auto-complete:** type a query, verify suggestions appear in the dropdown, click a suggestion and verify navigation
   - **Search results + infinite scroll:** search for a broad term, scroll to the bottom, verify the next batch loads automatically. If the dataset is small, use a small page size to guarantee multiple windows.
