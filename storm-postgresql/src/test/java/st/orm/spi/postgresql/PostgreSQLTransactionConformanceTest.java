...
/**
 * Runs the read-only transaction conformance suite against PostgreSQL. The suite commits, so it runs without the
 * per-test rollback and removes the rows it inserts.
 */
@StormTest(scripts = "/data.sql", rollback = false)
public class PostgreSQLTransactionConformanceTest extends AbstractTransactionConformanceTest {
